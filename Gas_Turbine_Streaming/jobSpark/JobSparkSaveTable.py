...
def predict_and_add_columns(batch_df, epoch_id):
    ...
    if not processed_data.empty:
        ...
        processed_data['actual_nox'] = nitro_values

        # Add the 'predictions' column to the Pandas DataFrame
        processed_data['predicted_nox'] = predictions.round(3).tolist()  #actual_nox, predicted_nox, read_timestamp

        # Add the 'nitro' column to the Pandas DataFrame
        processed_data['read_timestamp'] = timst_values

        # Select only the specified three columns for visualization
        result_df = processed_data[['actual_nox', 'predicted_nox','read_timestamp']]
        ...
        # Save the result to a PostgreSQL table
        result_df_spark \
            .write \
            .format("jdbc") \
