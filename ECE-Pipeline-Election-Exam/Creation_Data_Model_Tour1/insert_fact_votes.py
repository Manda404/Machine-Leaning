...
# Requête SQL
sql_query = """
INSERT INTO fact_votes (cod_dep, cod_candidat, inscrits, abstentions, pct_abs_ins, votants, pct_vot_ins, blancs,
...
FROM public.resultat_tourt1 rt
JOIN
    dim_departements dd ON rt."Code du département" = dd."Code du département"
JOIN
    DIM_CANDIDATS dc ON UPPER(rt."Nom") = UPPER(dc.Nom) AND UPPER(rt."Prénom") = UPPER(dc.Prénom);
"""
...
for num_col in range(1, 12):
    ...
    JOIN
        dim_departements dd ON rt."Code du département" = dd."Code du département"
    JOIN
        DIM_CANDIDATS dc ON UPPER(rt."Nom_{num_col}") = UPPER(dc.Nom) AND UPPER(rt."Prénom_{num_col}") = UPPER(dc.Prénom);
    """
